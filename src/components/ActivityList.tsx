...
export const ActivityList = ({ activities, dispatch }: ActivityListProps) => {
    const categorieName = useMemo(() => (category: Activity['category']) => categories.map(item => item.id == category ? item.name : ''), [activities])

    const isEmptyActivities = useMemo(() => activities.length === 0, [activities]);
    return (
        <>
            <h2 className="text-4xl font-bold text-slate-600 text-center">Meals & Activities</h2>
            {
                isEmptyActivities ?
                    <p className="py-10 text-center">No activities yet...</p> :
                        ...
                        <div key={activity.id} className="px-5 py-10 bg-white mt-5 flex justify-between shadow">
                            <div className="space-y-2 relative">
                                <p className={`absolute -top-8 -left-8 px-10 py-2 text-white uppercase font-bold ${activity.category === 1 ? 'bg-lime-500' : 'bg-orange-500'}`}>
                                    {categorieName(+activity.category)}
                                </p>
                                <p
